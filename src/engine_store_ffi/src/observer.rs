...
impl engine_tiflash::FFIHubInner for TiFlashFFIHub {
    fn get_store_stats(&self) -> engine_tiflash::FsStatsExt {
        debug!(
            "!!!!! self.engine_store_server_helper ois {:?} helper_ptr {}",
            self.engine_store_server_helper, self.engine_store_server_helper as *const _ as isize,
        );
...
fn retrieve_sst_files(snap: &raftstore::store::Snapshot) -> Vec<(PathBuf, ColumnFamilyType)> {
    let mut ssts: Vec<(PathBuf, ColumnFamilyType)> = vec![];
