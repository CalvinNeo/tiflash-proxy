...
use raftstore::store::peer_storage::{RAFT_INIT_LOG_INDEX, RAFT_INIT_LOG_TERM};
...
impl RegionChangeObserver for TiFlashObserver {
    fn on_region_changed(
        ...
        return
        if event == RegionChangeEvent::Bootstrap {
            match self.engine
                .get_msg_cf::<RaftApplyState>(engine_traits::CF_RAFT, &keys::apply_state_key(ctx.region().get_id()))
                .unwrap()
            {
                Some(_) => (),
                None => {
                    ...
                    self.engine.rocks.put_msg_cf(engine_traits::CF_RAFT, &keys::apply_state_key(ctx.region().get_id()), &apply_state);
                    ()
                }
            }
        }
