...
fn test_leadership_change() {
    ...
    for (old, new) in prev_states.iter().zip(new_states.iter()) {
        assert_eq!(old.0, new.0);
        assert_eq!(old.1.in_memory_apply_state, new.1.in_memory_apply_state);
        assert_eq!(old.1.in_memory_applied_term, new.1.in_memory_applied_term);
    }

    fail::remove("on_empty_cmd");
    // We need forward empty cmd generated by leadership changing to TiFlash.
    cluster.raw.must_transfer_leader(region.get_id(), peer_1.clone());
    let new_states = collect_all_states(&cluster, region_id);
    for (old, new) in prev_states.iter().zip(new_states.iter()) {
        assert_eq!(old.0, new.0);
        assert_ne!(old.1.in_memory_apply_state, new.1.in_memory_apply_state);
        assert_ne!(old.1.in_memory_applied_term, new.1.in_memory_applied_term);
    }
