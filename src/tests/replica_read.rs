...
fn test_read_index() {
    ...
    let p3 = new_peer(3, 3);
    pd_client.must_add_peer(r1, p3.clone());
    cluster.raw.must_put(b"k0", b"v0");
    pd_client.must_none_pending_peer(p2.clone());
    pd_client.must_none_pending_peer(p3.clone());
    let region = cluster.raw.get_region(b"k0");
    assert_eq!(cluster.raw.leader_of_region(region.get_id()).unwrap(), p1);
